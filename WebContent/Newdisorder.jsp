			...
			<div class="span12">
				...
				<%
					database db = new database();
						...
						if (list.isEmpty()) {
				%>
				<span class="changfont">您没有预订房间！</span>
				<%
					db.close();
				...
				<table class="table">
					...
					<tbody>
						...
						<tr>
							...
							<td><a
								href="changeorder.jsp?roomid=<%=list.get(i).getroom_num()%>&entertime=<%=list.get(i).getentertime()%>&lasttime=<%=list.get(i).getlasttime()%>&type=<%=type %>&price=<%=list.get(i).getprice() %>">修改</a>
								<a
								href="disorder.jsp?roomnum=<%=list.get(i).getroom_num()%>&entertime=<%=list.get(i).getentertime()%>&money=<%=list.get(i).getpay()%>">删除</a>
							</td>
						</tr>
						<%
							count++;
				...
				<div class="pagination pagination-centered pagination-small">
					<ul>
						<li><a href="Newdisorder.jsp?curpage=1">首页</a></li>
						<li><a href="Newdisorder.jsp?curpage=<%=curpage - 1%>">上一页</a></li>
						<li><a href="Newdisorder.jsp?curpage=<%=curpage + 1%>">下一页</a></li>
						<li><a href="Newdisorder.jsp?curpage=<%=pagecount%>">尾页
						</a></li>
