			...
			<div class="span12">
				...
				<table class="table">
					...
					<tr>
						...
						<td>
							<%
								out.print(temp.getidentification());
							%>
						</td>
						<td>
							<%
								out.print(temp.getpasswd());
							%>
						</td>
						<td><a
							href="update.jsp?username=<%=temp.getusername()%>&age=<%=temp.getage()%>&add=<%=temp.getaddress()%>&phone=<%=temp.getphone()%>&ide=<%=temp.getidentification()%>&passwd=<%=temp.getpasswd()%> ">修改</a></td>
					</tr>
					<%
						}
							}
