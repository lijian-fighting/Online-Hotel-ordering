		...
		<div class="row-fluid">
			<div class="span12">
				...
				<table class="table">
					...
					<tbody>
						...
						<tr>
							...
							<td><a href="pay.jsp?roomnum=<%=room.get(i).getroomNum() %>&entertime=<%=enter %>&lasttime=<%=last %>&type=<%=room.get(i).getroomType() %>">预订</a></td>
						</tr>
						<%
							count++;
											}
				...
				<div class="pagination pagination-centered pagination-small">
					<ul>
						<li><a
							href="Neworderroom.jsp?curpage=1&entertime=<%=enter%>&lasttime=<%=last%>&select1=<%=type%>&select2=<%=price%>">首页</a></li>
						<li><a
							href="Neworderroom.jsp?curpage=<%=curpage - 1%>&entertime=<%=enter%>&lasttime=<%=last%>&select1=<%=type%>&select2=<%=price%>">上一页</a></li>
						<li><a
							href="Neworderroom.jsp?curpage=<%=curpage + 1%>&entertime=<%=enter%>&lasttime=<%=last%>&select1=<%=type%>&select2=<%=price%>">下一页</a></li>
						<li><a
							href="Neworderroom.jsp?curpage=<%=pagecount%>&entertime=<%=enter%>&lasttime=<%=last%>&select1=<%=type%>&select2=<%=price%>">尾页
						</a></li>
